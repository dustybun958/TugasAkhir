            ...
            <div class="card-body">
                <form action="{{ route('updateArtikel',['id'=>$artikel->id]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror" rows="5" placeholder="Masukkan deskripsi artikel">{{ old('deskripsi', $artikel->deskripsi) }}</textarea>
                        @error('deskripsi')
                        <div class="alert alert-danger mt-2"> 
                          {{ $message }}
                        </div>
